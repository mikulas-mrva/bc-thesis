...
This section is based on Lévy's paper \emph{Axiom Schemata of Strong Infinity in Axiomatic Set Theory}, \cite{Levy60a}. It presents Lévy's general reflection principle and its equivalence to \emph{Replacement} and \emph{Infinity} under $\sf{S}$\footnote{See definition \bref{def:s}.}.
...
Let $N_0$ be defined as in definition \bref{def:levy_axiom_n0}, for \emph{Infinity} see definition \bref{def:infinity}.
...
Let $\sf{S}$ be a set theory defined in definition \bref{def:s}, $N_0$ a schema defined in definition \bref{def:levy_axiom_n0} and \emph{Replacement} a schema defined in definition \bref{def:replacement}.
\begin{theorem} 
...
% That means the following is an instance of \emph{Replacement} for $\varphi$ and~$x$~and it holds in $u$, so we get $\chi^{u}$, which means that $\chi$ holds by (iii).
% \beq
...
\item \emph{Extensionality} holds from lemma \bref{lemma:extensionality_in_transitive}.

\item \emph{Foundation} holds from lemma \bref{lemma:foundation_in_transitive}.
...
Let \emph{Infinity} and \emph{Replacement} be as defined in definition \bref{def:infinity} and definition \bref{def:replacement} respectively.
