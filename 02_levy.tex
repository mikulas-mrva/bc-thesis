...
The following is an axiom schema of complete reflection over $\sf{ZF}$, denoted as $N$. For every first-order formula $\varphi$ in the language of set theory with no free variables except for $x_1, \ldots , x_n$, the following is an instance of schema $N$.
\begin{equation}
...
Axiom schema $N_0$ is similar to $N$ defined above, but with $\sf{S}$ instead of $\sf{ZF}$. For every $\varphi$, a first-order fomula in the language of set theory with no free variables except $x_1, \ldots , x_n$, the following is an instance of $N_0$.
\begin{equation}
...
Let $\varphi = \forall x \exists y (y = x \cup \{x\})$. This clearly holds in $\sf{S}$ because given any set $x$, we always have $x \cup \{x\}$ via \emph{Powerset} and \emph{Specification}.
