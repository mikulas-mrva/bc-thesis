...
\subsection{$\sf{S} \proves (N_0\ \iff\ \emph{Replacement} \et \emph{Infinity})$} 
...
%We will heavily rely on lemma \ref{lemma:delta_0_abssoluteness}, which says that $\Delta_0$ formulas are absolute in transitive structures, in the following steps.
\bce[(i)]
...
\end{equation}
The formula $wf(x) = x \neq \emptyset \then \exists (y \in x) (\forall z \neg (z \in y \et z \in x))$\footnote{"wf" stands for well-founded.} is $\Delta_0$, \ref{lemma:delta_0_abssoluteness}.
...
\end{equation}
\emph{Pairing} also holds from limitness of $u$.
...
\end{equation}
\emph{Union} holds from transitivity of $M$ together with powerset TODO!!!


\item \emph{Subset / specification}: TODO!!!
...
% TODO pozor na zmatek ve jmenech promennych
