...
%\eeq
Let $\chi$ be an instance of the \emph{Replacement} schema for the $\varphi$ given. We want to verify in $\sf{S}$ that given a formula $\varphi$, the instance of $N_0$ for $\varphi$ implies $\chi$.
\begin{equation}
\begin{gathered}
...
Since it can be shown that $N_0$ is equivalent to $N''_0$ similar to $N''$ in (\ref{lemma:n_iff_n'}), which reflects any finite number of formulas to a single set, there is a set $u$ such that $Scm^{\sf{S}}(u)$ and all of the following hold:
\bce[(i)]
...
it maps elements of $x$ into $u$. From \emph{Specification}, we can find $y$, a~set of all images of the elements of $x$ via $\varphi$. That means that \emph{Replacement} holds in $u$, but we want more. So far, we have shown that 
\beq
(\forall x, p_1, \ldots, p_n \in u) \chi
\eeq
holds. By (iii) this is equivalent to 
\beq
(\forall x, p_1, \ldots, p_n \in u) \chi^{u}\mbox{.}
\eeq 
which is exactly 
\beq
(\forall x, p_1, \ldots, p_n \chi)^{u}\mbox{.}
\eeq
