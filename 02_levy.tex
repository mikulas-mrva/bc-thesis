...
We can define a power set by a $\Pi_1$-formula\footnote{$z \subseteq x$ is a $\Delta_0$-formula.}, which is then downward absolute by lemma (\ref{lemma:downward_absoluteness}).
\beq
% \forall x \exists y \forall z (z \subseteq x \iff z \in y)
% \foreach z (y \in \powerset{x} \iff 
% TODO \Pi_1-definice \powerset{x} !
\forall z (z \subseteq x \iff z \in y)
\eeq
It is easy to see that since \emph{Powerset} holds in $V$, it also holds in $V_\lambda$. Note that limitness of $\lambda$ is needed here because $rank(\powerset{x}) = rank(x)+1$.


TODO !!!!
%Let $\varphi$ denote the formula $y \in \power{x} \iff y \subset x$.
