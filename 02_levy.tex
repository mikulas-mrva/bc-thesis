...
Scm^{\sf{Q}}(V_\kappa)
\end{equation}
We write $In^{\sf{Q}}(\kappa)$.\footnote{To be able to define $V_\kappa$, we need to work in a logic that contains the \emph{Replacement Schema} or any of it's equivalents. It should be noted that we don't work in an arbitrary theory \sf{Q}, but in \sf{ZF}, which contains the \emph{Replacement Schema}. 
...
% Note that this is a restatement of both Lévy's $N$ and $N_0$ from the previous chapter\footnote{see \bref{def:levy_axiom_n} and lemma \bref{def:levy_axiom_n0}}. We prefer to call it \emph{First-order reflection} so it complies with how other axioms and schemata are named. \footnote{We will not use the name $N_0$, because it might be confusing to work $N_0$ and $M_0$ where $M_0$ is a set and $N_0$ is an axiom schema.} 
...
% We will now prove the equivalence of \emph{First-order reflection} with the \emph{Replacement Schema} and the \emph{Axiom of Infinity} in $\sf{S}$ in two parts. First, we will show that \emph{First-order reflection} is a theorem of $\sf{ZFC}$, then we shall show that the second implication, which proves the \emph{Axiom of Infinity} and the \emph{Replacement Schema} from \emph{First-order reflection}, also holds.
...
It is clear from relativisation\footnote{See definition \bref{def:relativization}. This only holds for relativization to $M, \in \cap M \times M$, not $M, R$ for an arbitrary $R$.} that \eref{equation:levy_theorem_i} holds for both cases, $(x_1 = x_2)^M \iff (x_1 = x_2)$ and $(x_1 \in x_2)^M \iff (x_1 \in x_2)$.
...
Since $V_\lambda$ is a~transitive set, by proving $\bold{(iii)}$ we also satisfy $\bold{(ii)}$. To do so, we only need to look at part $\bold{(ii)}$ of lemma \bref{lemma:reflection_lemma}. All of the above proof also holds for $M = V_lambda$. 

To finish part $\bold{(iv)}$, we take $M$ of size $\leq |M_0| \cdot \aleph_0$, which exists due to part $\bold{(iii)}$ of lemma \bref{lemma:reflection_lemma}, the rest being identical.
\end{proof}
...
according to definition \bref{def:subset}, $y \subset x$ is $\Delta_0$, so for any given $x, y \in V_\lambda$, $y = \power{x} \iff \langle V_\lambda, \in \rangle~\models~y = \power{x}$.
...
\emph{First-order reflection} is equivalent to the \emph{Axiom of Infinity} $ \et $ the \emph{Replacement Schema} under $\sf{S}$.
\end{theorem}
% lemma:model_of_s?
\begin{proof}
...
$\bold{\emph{First-order reflection} \then the \emph{Axiom of Infinity}}$
...
$\bold{\emph{First-order reflection} \then the \emph{Replacement Schema}}$
...
We have also shown that it can be used instead of the the \emph{Axiom of Infinity} and the \emph{Replacement Schema} scheme, but $\sf{ZFC}\ +\ \emph{First-order reflection}$ is a~conservative extension of $\sf{ZF}$. Besides being a~starting point for more general and powerful statements, it can be used to show that $\sf{ZF}$ is not finitely axiomatizable. This follows from the fact that \emph{Reflection} gives a~model to any consistent finite set of formulas. % or their conjunction? % nedokazal jsem verzi pro n formuli!
So if $\varphi_1, \ldots, \varphi_n$ would be the axioms of $\sf{ZFC}$, $\emph{Reflection}$ would prove that every model of $\sf{ZFC}$ contains a smaller model of $\sf{ZFC}$, which would in turn contradict the Second Gödel's Theorem\footnote{See chapter \bref{section:inaccessibility} for further details.}.
