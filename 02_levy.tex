...
\item \emph{Powerset}: \\(see (\ref{def:powerset})) %fixme
\begin{equation}
\forall x \exists y \forall z (z \subseteq x \iff z \in y)
\end{equation}
\emph{Powerset} holds from limitness of $u$ by the argument used in the other implication of this lemma. % TODO odflakly
...
\end{equation}
Given two sets $x, y \in u$, we want to show that $z = \{x, y\}$ is also an element of $u$.
\beq
z = \{x, y\} \iff x \in z \et y \in z \et (\forall q \in z)(q = x \lor q = y)
\eeq
So $(z = \{x, y\})$ is $\Delta_0$, thus, by lemma (\ref{lemma:delta_0_absoluteness}).
\beq
$z = \{x, y\} \iff (z = \{x, y\})^u$
\eeq

\item \emph{Specification}: \\ %fixme
Given a set $x$, for any $\varphi$, all the elements of $x$ satisfying $\varphi$ form a subset of $x$, which is an element of $\power{x}$ and thus also a subset of $u$ by \emph{powerset} and transitivity of $u$.
\ece
\end{proof}
