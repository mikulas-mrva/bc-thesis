...
This clearly holds in \sf{S} because given a set~$x$, there is a set $y = x \cup \{x\}$ obtained via \emph{Axiom of Pairing} and \emph{Axiom of Union} and since the sets obtained via these axioms are definable via $\Delta_0$–formulas, there are absolute in transitive structures thanks to Lemma \bref{lemma:delta_0_absoluteness}. %\emph{Axiom of Powerset} and \emph{Specification Schema}.
...
% That means the following is an instance of the \emph{Replacement Schema} for $\varphi$ and~$x$~and it holds in $u$, so we get $\chi^{u}$, which means that $\chi$ holds by (iii).
% \beq
...
What we have just proved is only a single theorem from Lévy's aforementioned article, we will introduce other interesting results, dealing with inaccessible and Mahlo cardinals, in chapter 3.
...
\begin{equation}
\mbox{``$\lambda$ is a limit ordinal''} \then \langle V_\lambda, \in \rangle~\models~\sf{S} % proc ne?
\end{equation}
\end{lemma}
...
We have shown that $\emph{Reflection}$ for first–order formulas, is a~theorem of \sf{ZFC}. %, which means that it won't yield us any large cardinals. 
...
So if $\varphi_1, \ldots, \varphi_n$ would be the axioms of \sf{ZFC}, \emph{Reflection} would prove that every model of \sf{ZFC} contains a smaller model of \sf{ZFC}, which would in turn contradict the Second Gödel's Theorem. % TODO je to to co delal montague?
...
In the next section, we will try to generalise \emph{Reflection} in a~way that transcends \sf{ZFC} and yields some large cardinals.
\newpage
% =============================================================
