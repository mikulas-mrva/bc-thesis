...
\subsubsection{Relativisation and Absoluteness} % and absoluteness?
...
\ece
\end{definition}
When $R=\in\cap(M \times M)$, we usually write $\varphi^M$ instead of $\varphi^{M, R}$
...
\item A formula $\varphi$ is $\Pi_n+1$ if it is logically equivalent to a formula $\varphi'$ such that $\varphi' = \foreach x \psi$ where $\psi$ is a $\Sigma_n$-formula for any $n < \omega$.
\item A formula $\varphi$ is $\Sigma_n+1$ if it is logically equivalent to a formula $\varphi'$ such that $\varphi' = \foreach x \psi$ where $\psi$ is a $\Pi_n$-formula for any $n < \omega$.
\ece
\end{definition}
...
\begin{proof}
This will be proven by induction over the complexity of a given $\Delta_0$ formula $\vaphi$. Let $M$ be an arbitrary transitive class. Suppose, that 
...
For a limit ordinal $A \subseteq \lambda$, we say that $A$ is closed in $\lambda$ iff for every non-zero ordinal $\alpha < \lambda$: if $A \cap \alpha$ is unbounded in $\alpha$ then $\alpha \in A$.
\end{definition}
