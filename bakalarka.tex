...
Ze prvoradova reflexe je theorem ZFC, vys uz je to nezavisle tvrzeni. (mozna dk. nezavislosti?)
\medskip
...
TODO trochu vyjasnit parametry
...
\begin{equation}
(\forall x, y, z (\varphi(x, y, u_1, \ldots, u_n) \& \varphi(x, z, u_1, \ldots, u_n) \then y = z)) \then (\forall X \exists Y (\forall y\ (y \in Y \iff \exists x (x \in X \& \varphi(x, y, u_1, \ldots, u_n)))))
\end{equation}
TODO co je dom?
...
Which is exactly the Replacement Schema.
\end{proof}
