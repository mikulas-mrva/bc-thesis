...
Notice, that reflection in a way counterpart to compactness. Compactness argues that for an infinite\footnote{Countable?} set of formulas, if every finite subset yields a model, then so does the whole set. Reflection, on the other hand, says that while the whole set has no model in the underlying theory, every finite subset does have a model.

%posl. vetu do jine sekce?
...
To see that there is a way to transcend $\sf{ZF}$, let us briefly show how a model of $\sf{ZF}$ can be obtained in $\sf{ZF} + "second-order reflection"$. This will be more closely examined in section \ref{section:inaccessibility}.
Since 
...
$\kappa$ is \emph{weakly inaccessible} $\iff$ it is \emph{regular} and \emph{limit}.
\end{definition}
