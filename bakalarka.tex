...
\begin{equation}
N \defeq \exists u (Scm^{\sf{ZF}}(u)\ \&\ \forall x_1, \ldots , x_n (x_1, \ldots , x_n \in u \then \varphi \iff \varphi^{u}))
\end{equation}
...
\begin{equation}
N_0 \defeq \exists u (Scm^{\sf{S}}(u)\ \&\ \forall x_1, \ldots , x_n (x_1, \ldots , x_n \in u \then \varphi \iff \varphi^{u}))
\end{equation}
where $\varphi$ is a~formula which contains no free variables except for $x_1, \ldots , x_n$.
\end{definition}
...
For an arbitrary $\varphi$, $N_0$ gives us $\exists u Scm^{\sf{S}}(u)$, but from lemma \ref{lemma:scm_s_is_limit}, we know that this $u$ is a limit ordinal. This effectively satisfies \emph{Infinity}.
\end{proof}
...
Let $\chi$ be an instance of \emph{Replacement} schema for this $\varphi$ which is what we want to prove:
\begin{equation}
\begin{gathered}
