...
\begin{equation}
\exists u (Scm^{\sf{S}} \& x_1, \ldots , x_n \in u \then \varphi \iff Rel(u, \varphi))
\end{equation}
where $\varphi$ is a formula which does not contain free variables except $x_1, \ldots , x_n$.
\end{definition}

\begin{theorem}
In \sf{S} the axiom schema of replacement in conjuction with the axiom of infiniy is equivalent to the schema $N_0$.
\end{theorem}

\begin{proof}  % sezen toho montagua!!!  Journal of symbolic logic, 1961
