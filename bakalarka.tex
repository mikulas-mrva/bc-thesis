...
All proofs were a~model that of $\sf{ZFC}$ was $V_\kappa$ (notated as $R(\kappa)$ at the time) for some cardinal $\kappa$, which means that $\kappa$ is a~inaccessible cadinal.  % fakt? mozna ze my to proste delame ciste pres semanticky dusledek, ale jinak to vyslo nastejno.
...
% This is a small intermezzo to clarify notation for the following chapter, which will use some basic model theory.

Unless stated otherwise, we will work in structures for the language $L_\in$ of the form $\langle M, \in \cap (M \times M) \rangle$ where $M$ is a domain, usually a set\footnote{Unless explicitly stated otherwise. It might as well be a class, but in that case, formally establishing a satisfcation relation is impossible. This problem would be dealt with individually for each situation occuring.}. This structure will be notated either as $\langle M, \in \rangle$ or just $M$\footnote{In situations like $M \models \varphi$, $\varphi$ being a first-order formula.}. If additional unary predicates are required in the language, $\langle M, \in \cap (M \times M), R_1, \ldots, R_n \rangle$ is written as $\langle M, \in, R_1, \ldots, R_n \rangle $ for brevity.

The satiscation relation $\models$ for first-order formulas is defined as usual.

% . Reflection theorems asdasd Tarski Berkley model-theoretic methods in set theory.

%This notation is used for example in \cite{KanBook}.
...
TODO nejaka veta ze kdyz jsou $\Pi^1_0$-indescribable, jsou i $\Pi^m_n$-indescribable pro $m \leq 1, n \leq 0$? Drake? Adding spare quantifiers?
...
\subsection{Weakly Compact Cardinal}

We will examine another large cardinal, one that is defined by many different properties, while we will mention three of them\footnote{(partition, extension, pi11-indesc.?)}, proving their mutual equivalence is out of our scope of interest.

%Weakly Compact Cardinal was first described by Erdős and Tarski in \cite{erdos_tarski_61}
The notion of weak compactness comes from Tarski's work with infinitary languages and his subsequent generalisation of compactness for those languages. We're not interested in particular details on how exactly is the language constructed, so let's skim though it briefly, just to know where this cardinal's name comes from. For a bit more thorough description and references, see \cite{KanamoriBook}.

Let's suppose that $L_{\lambda, \mu}$ is a language in which formulas are allowed to contain conjunctions and disjunctions of $\alpha$ subformulas for any $\alpha < \lambda$ and quantifiers may quantify over $\beta$ variables for any $\beta < \mu$. Formulas are allowed to contain less than $\mu$ free variables and the language itself contains $max(\{\lambda, \mu\})$ variables. Predicate and function symbols as well as constants are finitary\footnote{In a sense of "the same as in finitary first-order logic"}. Now we can say a collection of $L_{\lambda, \mu}$-formulas is \emph{satisfiable} iff it has a model\footnote{With conjunction, disjuction and quantification interpreted accordingly.} and it is $\kappa$\emph{-satisfiable} iff every collection of formulas of cardinality less than $\kappa$ formulas is satisfiable. This allows us to state the original definition of a weakly compact cardinal.

\begin{definition}
An uncountable cardinal $\kappa$ is weakly compact iff any collection of $L_{\kappa, \kappa}$ sentences using at most $\kappa$ non-logical symbols,if $\kappa$-satisfiable, is satisfiable.
\end{definition}

Now that we know why a \emph{Weakly compact cardinal} is weakly compact, let's state a definition we will actually use.
...
TODO vysledek o L: \cite{scott_measurable_constructible}.
...
\begin{equation}
\begin{gathered}
Def(X) := \{\{{y | x }\in X \land \langle X, \in \rangle \models \varphi(y, z_1,\ldots,z_n) \} |\\
\mbox{ }\varphi\mbox{ is a~first-order formula, }z_1,\ldots,z_n \in X \}
\end{gathered}
\end{equation}
\end{definition}
