...
This text assumes the knowledge of basic terminology and some results from first-order predicate logic, see \cite{hamiltonBook}. We won't introduce the notions of \emph{language}, \emph{function symbol}, \emph{predicate}, \emph{term}, \emph{model} and \emph{interpretation} that are used in (\ref{def:satisfaction}).
...
\end{equation}
The least set satisfying (\ref{eq:axiom_of_infinity}) is denoted $\omega$.
\end{definition}
...
Given a set~$x$, the \emph{powerset of $x$}, denoted $\power{x}$ and satisfying the definition (\ref{def:powerset}), is defined as follows:
\begin{equation}
...
\eeq
\end{definition}
In other words, $B$ is cofinal in $A$ iff it is unbounded in $A$. % todo fakt?
...
% TODO fixme
% TODO asi staci rict ze existuje takovy limitni ordinal?
...
\end{definition}
% TODO lemma o pruniku konecne mnoha club setu? % nakonec netreba
