...
We will now shortly review the basic notions that allow us to define the \emph{ZermeloâFraenkel} set theory.
...
We will often write something like ``$M$ is a limit ordinal'', it should always be clear that this can be rewritten as a formula that was introduced earlier.
...
\eeq
$\emptyset$ is a set due to \emph{Specification}, there is only one such set due to \emph{Extensionality}. %While the empty set could also be defined by the formula $\forall y (y \in \emptyset \iff \neg(y = y))$, the version we use is $\Delta_0$, which we will find useful later. The two definitions yield the same set for every $x$ given because of \emph{Extensionality}.
% TODO odkomentovat a pridat zduvodneni proc to neni vl. trida?
\end{definition}
...
\end{equation}
\end{definition}
% TODO nedat tam univerzalni uzaver?
...
\end{equation}
\end{definition}
We call such $f$ a \emph{choice function}. One might be unsettled by the fact that this definition quantifies over functions, which are generally classes, but in this particular case, since $dom(f) = x$ and $x$ is a set, $f$ is also a set because a function is in fact a class of tuples and there is an obvious 1-1 mapping between those and the elements of $dom(f)$.
...
Given two different ordinals $\alpha, \beta$, we will write $\alpha <Â \beta$ for $\alpha \in \beta$, see Lemma 2.11 in \cite{JechBook} for technical details.

\begin{definition}{(Non-Zero Ordinal)} % ok 
We say an ordinal $\alpha$Â is \emph{non-zero} iff $\alpha \neq \emptyset$.
\end{definition}
...
A non-zero ordinal $\alpha$Â is called a \emph{limit ordinal} iff it is not a successor ordinal.
\end{definition}
...
%\begin{definition}{(LÃ©vy's Hierarchy)}\\
...
We say a cardinal $\kappa$ is \emph{regular} iff $cf(\kappa) = \kappa$. % TODO chyba podle honzika, watt?
\end{definition}
...
\end{equation}
\end{definition}
If \emph{GCH} holds (for example in GÃ¶del's $L$, see chapter 3), the notions of limit cardinal and strong limit cardinal are equivalent.
...
We will also use $\langle M, R \rangle \models \varphi(p_1, \ldots, p_n)$ and $\varphi^M(p_1, \ldots, p_n)$ interchangably.
