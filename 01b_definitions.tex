...
\item \emph{Axiom of Powerset} (see definition \bref{def:powerset})
\ece
\end{definition}
...
From relativization, $(\exists x \psi(x))^M \iff (\exists x \in M) \psi^M(x)$. Since the hypotheses makes it clear that $\psi^M \iff \psi$, we get 
\beq
...
Structures will be denoted $\langle M, \in, R \rangle$ where $M$ is a domain, $\in$ stands for the standard membership relation, it is assumed to be restricted to the domain\footnote{To be totally explicit, we should write $\langle M, \in \cap M \times M, R \rangle$.}, $R \subseteq M$ is an unary relation on the domain. %When $R$ is not needed, we can as well only write $M$ instead of $\langle M, \in \rangle$.
