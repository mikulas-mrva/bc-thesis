...
\eeq
$\emptyset$ is a set due to \emph{Specification}. While the empty set could also be defined by the formula $\forall y (y \in \empty \iff \neg(y = y))$, the former version is $\Delta_0$, which we will find useful later on. The two definitions yield the same set for every $x$ given thanks to \emph{extensionality}.
\end{definition}
