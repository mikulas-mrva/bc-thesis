...
Speaking of formulas, we will often use syntax like "$M$ is a limit ordinal". It should be clear that this statement can be rewritten as a formula that was introduced earlier in the text.
...
For an arbitrary set $x$, the empty set, written as $\emptyset$, is defined by the following formula
\beq
(\forall y \in x)(y \in \emptyset \iff \neg(y = y))
\eeq
We know that $\emptyset$ is a set from \emph{specification}. While it could also be defined by "$\forall y (y \in \empty \iff \neg(y = y))$", we have decided to use the bounded version because it's $\Delta_0$, which we will find useful later on. The two definitions yield the same set for every $x$ given thanks to \emph{extensionality}.
\end{definition}
...
\begin{equation}
\forall x \exists y \forall z (z \subseteq x \iff z \in y) %ok
\end{equation}
\end{definition}
...
% V_\alpha is the set of all sets having ranks less than \alpha !
...
\begin{equation} % The rank of a set S is the smallest α such that {\displaystyle S\subseteq V_{\alpha }\,.} S\subseteq V_{\alpha }\,.
x \in V_{\alpha+1}
\end{equation}
\end{definition}
...
For formal details as well as why every set can be well-ordered assuming \emph{Choice}, and therefore has a cardinaliy, see \cite{JechBook}. % proc je to tady?
...
\begin{equation}
(\exists \alpha \in Ord)(\kappa = \aleph_\alpha) % neni in ord redundantni, protoze je V_x def jenom pro ordinalni x?
\end{equation}
\end{definition}
