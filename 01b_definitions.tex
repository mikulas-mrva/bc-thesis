...
This text assumes the knowledge of basic terminology and some results from first-order predicate logic, see any entry-level book on logic, e.g. \cite{hamiltonBook}. We won't introduce the notions of \emph{language}, \emph{function symbol}, \emph{predicate}, \emph{term}, \emph{model} and \emph{interpretation} that are used in definition \bref{def:satisfaction}.
...
Given a set~$x$, the \emph{powerset of $x$}, denoted $\power{x}$ and satisfying the definition \bref{def:powerset}\, is defined as follows:
\begin{equation}
...
%\begin{equation}
%\begin{gathered}
%\forall x \exists f ((f \mbox{ is a function with } dom(f) = x \setminus \{\emptyset\})\\
%\et \forall y ((y \in y \et y \neq \emptyset) \then f(y) \in y))
%\end{gathered}
%\end{equation}
\end{definition}
One might be unsettled by the fact that this definition quantifies over functions, which are generally classes, but in this particular case, since $dom(f) = x$ and~$x$~is a set, $f$ is also a set due to \emph{Replacement}\footnote{If the underlying theory includes of implies \emph{Replacement}.}.% because a function is in fact a class of tuples and there is an obvious 1–1 mapping between those and the elements of $dom(f)$.
...
\item \emph{Extensionality} (see definition \bref{def:extensionality})
\item \emph{Specification} (see definition \bref{def:specification})
\item \emph{Foundation} (see definition \bref{def:foundation})
\item \emph{Pairing} (see definition \bref{def:pairing})
\item \emph{Union} (see definition \bref{def:union})
\item \emph{Powerset} (see definition \bref{def:powerset})
\ece
\end{definition}
...
\bce[(i)]
\item \emph{Replacement} schema (see definition \bref{def:replacement})
\item \emph{Infinity} (see definition \bref{def:infinity})
\ece
...
Let $\omega$ be the set defined by definition \bref{def:omega}.
...
\eeq
\end{definition}
In other words, $B \subseteq A$ is cofinal in $A$ iff it is unbounded in $A$.

% TODO bylo to blbe, je opraveno?
...
Atomic formulas are always absolute by the definition of relativisation, see definition \bref{def:relativization}.
...
From relativization and lemma \bref{lemma:delta_0_absoluteness}, $\varphi^M(p_1, \ldots, p_n) \iff (\forall x \in M)\psi(p_1, \ldots, p_n, x)$.
...
From relativization and lemma \bref{lemma:delta_0_absoluteness}, $\varphi^M(p_1, \ldots, p_n) \iff (\exists x \in M)\psi(p_1, \ldots, p_n, x)$.
