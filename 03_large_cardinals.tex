...
. $\bigtcup Y$ is a limit ordinal. If it were a successor ordinal, suppose that $\alpha+1 = \bigcup Y$, then $\alpha \in \bigcup Y$, which means that there is some $x$ such that $\alpha \in x \in Y$. But the least such $x$ is $\alpha+1$, so $\bigcup Y \in Y$.
...
\eeq
because we have proven in the last section that for an unaccessible $\kappa$, $V_\kappa \models \sf{ZFC}$. % TODO ref



% TODO ekvivalence s reflexi
