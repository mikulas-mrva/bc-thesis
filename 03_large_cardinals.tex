...
Because we have extended the underlying logic to higher types of variables, in order to make some meaningful statements in it, we will now define the satisfaction relation for structures.\footnote{Note that it is formally undefinable even in first order logic due to Tarski's result, see \cite[Theorem 12.7]{JechBook} for details.}
\begin{definition}{(Satisfaction)}\label{def:satifsfaction}\\

\end{definition}

\begin{definition}{(Reflection)}\label{def:reflection_2}\\
Let $\varphi(P)$ be a $\Pi^n_m$-formula with one free variable of type type 2 denoted $P$. We say $\varphi(P)$ reflects in $V_\kappa$ if for every $P \sub V_\kappa$ there is an ordinal $\alpha<\kappa$ such that the following holds:
\begin{equation}
\begin{gathered}
...
TODO def Con!
\begin{theorem}
The existence of the inaccessible cardinal is compatible with 
%\begin{equation}
...
\begin{theorem}
\begin{equation}
Con(L + \exists \kappa (\kappa \mbox{" is a Mahlo Cardinal"})) then Con(L)
\end{equation}
\end{theorem}
