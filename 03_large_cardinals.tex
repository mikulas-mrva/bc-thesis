...
For the definition of a $\Pi^m_n$-formula and a $\Sigma^m_n$-formula, see (\ref{def:analytical_hierarchy}).
...
\ece
\end{theorem}
For a proof, see Proposition 6.5 in \cite{KanamoriBook}. % TODO emph{Proposition }?
...
\end{equation}
\end{theorem}
For a proof, see Proposition 6.6 in \cite{KanamoriBook}.
...
Without providing a proof, we will introduce two important results established by Gödel in his aforementioned article. \sf{ZF} stands for Zermelo–Fraenkel set theory as introduced in (\ref{def:zf}).
