...
To see that this notion is based in reflection, let us recall the opening quote by Gödel which says that \emph{``The Universe of sets cannot be uniquely characterised (i. e. dis- tinguished from all its initial elements) by any internal structural property of the membership relation in it (...)''}. A cardinal $\kappa$ is $\Pi^m_n$-indescribable\footnote{This holds for $\Sigma^m_n$-formulas alike.} iff every $\Pi^m_n$-formula fails to describe $V_\kappa$ and describes an initial segment instead.
...
\begin{lemma}
Let $\kappa$ be a cardinal, the following holds for any $n \in \omega$. $\kappa$ is $\Pi^1_n$-indescribable iff $\kappa$ is $\Sigma^1_{n+1}$-indescribable
\end{lemma}
...
Then there is, by lemma (\ref{lemma:inaccessible_clubset}) a club set of ordinals $\alpha$ such that $V_\alpha$ is an elementary substructures of $V_\kappa$. 
...
%\end{gathered}
%\end{equation}
where $P$ is a type 2 variable and the rest are type 1 variables,  ``$P$ is a function'' is a first-order formula defined in (\ref{def:function}). 
...
\begin{equation}
\langle V_\kappa, \in, C \rangle~\models~Inac \et \mbox{``$C$ is unbounded''}\footnote{``$C$ is unbounded'' is a first-order formula, see (\ref{def:unbounded_class}).}
\end{equation}
...
Since we have assumed that $\kappa$ is $\Pi^1_1$-indescribable, $\varphi$ is a $\Pi^1_1$-formula and $\varphi \et \mbox{``$C$ is unbounded''}$ is equivalent to a $\Pi^1_1$-formula, there must be an ordinal $\alpha$ that satisfies
\begin{equation}
\langle V_\alpha, \in, C \cap V_\alpha \rangle~\models~Inac \et \mbox{``$C$ is unbounded''}
\end{equation}
which implies that $\alpha$ is inaccessible.

To be finished, we need to verify that $\alpha \in C$, which is clear from the fact that $C$ is a club set in $\kappa$ and it is unbounded in $\alpha$.
\end{proof}
...
We say a cardinal $\kappa$ has the \emph{extension property} iff for all $U \subset V_\kappa$ there exists a transitive set $X$ with $\kappa \in X$, and a subset $S \subset X, such that $(V_\kappa, \in, U)$ is an elementary substructure of $(X, \in, S)$. Here, U and S are regarded as unary predicates.s
\end{definition}
