...
But this formulation is too vague, we should be very careful with the notion of property. The first problem that comes to mind is that ``being the set of all sets'' must not be considered a property in this sense, otherwise we run into the well known paradox of Russell. 
This is a problem that leads to the foundations of set theory and shows that reflection is a phenomenon that is very closely connected to the very foundations. 
The connection is exemplified by the fact that the very first use of reflection in a mathematical proof can be found in Gödel's paper \cite{Godel1940consistency}. 
Furthemore, Lévy's article that is central to this thesis is concerned primarily with the so called \emph{strong axioms of infinity}, which are axioms that imply the existence of $\omega$ 
% TODO asi rekni co je omega
% TODO proc jsou nektery velky kardinaly jako omega
% TODO nejakej Godeluv cotat o strong infinity
...
After introducing the elementary theoretical tools required for this task in the rest of this chapter, in chapter 2, we will review the \emph{Reflection theorem} as formulated by Azriel Lévy in his 1960 paper \emph{Axiom Schemata of Strong Infinity in Axiomatic Set Theory}\cite{Levy60a}. We will first show Lévy's original proof and surrounding theory and then offer contemporary restatement that is base on the version used by Jech in \cite{JechBook}. This chapter is meant to make the reading of Lévy's aforementioned paper easier and more comprehensible to today's reader.

The next chapter, \emph{Reflection And Large Cardinals},  %Since aim of Lévy's article wasn't reflection itself, but axiomatic set theories 
